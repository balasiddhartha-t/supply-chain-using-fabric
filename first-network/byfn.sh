...
function networkUp() {
  ...
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    ...
    export BYFN_CA1_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/jeep.fca.com/ca && ls *_sk)
    export BYFN_CA2_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/jeepdealer.fca.com/ca && ls *_sk)
    export BYFN_CA3_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/jeeplogistics.fca.com/ca && ls *_sk)
    export BYFN_CA4_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/jeeptrucking.fca.com/ca && ls *_sk)
    export BYFN_CA5_PRIVATE_KEY=$(cd crypto-config/peerOrganizations/cbp.gov/ca && ls *_sk)
  fi
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for JeepMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/JeepMSPanchors.tx -channelID $CHANNEL_NAME -asOrg JeepMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for JeepMSP..."
    exit 1
  fi

  echo
  echo "#######################################################################"
  echo "#######    Generating anchor peer update for JeepdealerMSP   ##########"
  echo "#######################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/JeepdealerMSPanchors.tx -channelID $CHANNEL_NAME -asOrg JeepdealerMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo
  echo "###########################################################################"
  echo "#######    Generating anchor peer update for JeeplogisticsMSP   ##########"
  echo "###########################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/JeeplogisticsMSPanchors.tx -channelID $CHANNEL_NAME -asOrg JeeplogisticsMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for JeeplogisticsMSP..."
    exit 1
  fi
  ...
  echo
  echo "###########################################################################"
  echo "#######    Generating anchor peer update for JeeptruckingMSP   ##########"
  echo "###########################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/JeeptruckingMSPanchors.tx -channelID $CHANNEL_NAME -asOrg JeeptruckingMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for JeeptruckingMSP..."
    exit 1
  fi
  ...
  echo
  echo "################################################################"
  echo "#######    Generating anchor peer update for CbpMSP   ##########"
  echo "################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/CbpMSPanchors.tx -channelID $CHANNEL_NAME -asOrg CbpMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for CbpMSP..."
    exit 1
  fi
